...
void selective_repeat(Frame *frames, int frame_count, int window_size) {
    int base = 0;
    int next_seq_num = 0;
    int *acked = (int *)calloc(frame_count, sizeof(int));
    ...
    while (base < frame_count) {
        ...
        while (base < frame_count && acked[base]) {
            base++;
        }
    }
...
int main() {
    ...
    selective_repeat(frames, frame_count, window_size);
